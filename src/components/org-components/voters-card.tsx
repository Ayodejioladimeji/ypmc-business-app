...
const VotersCardUpload = () => {
    const [uploading, setUploading] = useState(false);
    const { state, dispatch } = useContext(DataContext);
    const uploadedFile = state.ninUploadedFile;

    const handleImageUpload = (file: UploadedFile) => {
        dispatch({ type: ACTIONS.NIN_UPLOADED_FILE, payload: file });
    };

    const removeImage = () => {
        dispatch({ type: ACTIONS.NIN_UPLOADED_FILE, payload: null });
    };
    ...
    return (
        <View style={styles.container}>
            <Text style={styles.label}>Voter’s Card, NIN or Passport (optional)</Text>
...
export default VotersCardUpload;
