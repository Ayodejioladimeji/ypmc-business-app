...
import { View, Text, TouchableOpacity, Image, StyleSheet, Platform, Alert, ActionSheetIOS } from "react-native";
...
const ElectricityBill = () => {
    ...
    const { electricityBillUploadedFile: uploadedFile } = state

    const handleImageUpload = (file: UploadedFile) => {
        dispatch({ type: ACTIONS.ELECTRICITY_BILL_UPLOADED_FILE, payload: file })
    };

    const removeImage = () => {
        dispatch({ type: ACTIONS.ELECTRICITY_BILL_UPLOADED_FILE, payload: null })
    };
    ...
    const uploadImage = async (fromCamera = false) => {
        ...
        if (!result.canceled) {
            ...
            try {
                ...
                formData.append("documentType", "ELECTRICITY_BILL");
                formData.append("category", "BUSINESS");

                const response = await axios.post(
                    `${process.env.EXPO_PUBLIC_BASE_URL}/kyc/partner/upload`,
                    formData,
                    {
                        headers: {
