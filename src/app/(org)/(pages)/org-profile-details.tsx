...
import UpdateProfile from "@/components/org-components/update-profile";
    ...
    if (state?.token) {
      const getProfile = async () => {
        ...
        if (res?.status === 200 || res?.status === 201) {
          ...
          console.log("mine", res?.data?.data)
        }
  ...
  const handleSave = async () => {
    ...
    try {
      const res = await fetch(
        `${process.env.EXPO_PUBLIC_BASE_URL}/partner/upload-profile-image`,
        {
          method: "POST",
          headers: {
          ...
          <View style={{ marginTop: 50, gap: 12 }}>
            <View style={styles.field}>
              <View>
                ...
                <Text style={styles.fieldValue}>{user?.user?.companyName}</Text>
              </View>
            ...
            <View style={styles.field}>
              <View>
                <Text style={styles.fieldText}>Company Address</Text>
                  <Text style={styles.fieldValue}>{user?.metadata?.address?.street} {user?.metadata?.address?.state} {user?.metadata?.address?.country}</Text>
              </View>
