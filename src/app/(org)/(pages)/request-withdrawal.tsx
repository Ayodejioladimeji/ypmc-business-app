...
import { z } from "zod";
import AddBank from "@/components/org-components/add-bank";
...
import Withdraw from "@/components/org-components/withdraw";
...
export default function Payment() {
    ...
    useEffect(() => {
        if (state?.token) {
            const getBanks = async () => {
                const res = await GetRequest("/bank-accounts/partner", state?.token);
    ...
    const handleDelete = async () => {
        ...
        const res = await DeleteRequest(`/bank-accounts/partner/${id}`, state?.token);
        if (res?.status === 200 || res?.status === 201) {
                ...
                <View style={{marginTop:20}}>
                    <View style={{ paddingHorizontal: 16, alignItems:'center', justifyContent:'center'}}>
                        <Text style={{ fontSize: s(12), color: colors.mutedForeground, marginBottom:20 }}>
                            Wallet Balance ₦{formatMoney(wallets?.walletBalance) || 0}
                        </Text>
                        ...
                        <Text style={{ fontSize: s(20), marginBottom: 10, fontWeight: 700 }}>
                            ₦{wallets?.totalWithdrawn || 0}
                        </Text>

                        <Text style={{ fontSize: s(12), color: 'red', marginBottom: 20 }}>
                            Comission (10%) : - ₦{formatMoney(Number(wallets?.walletBalance) * 0.1)}
                        </Text>
