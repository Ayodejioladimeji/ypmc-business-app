...
const dayMapping: Record<string, Day> = {
 ...
 const BusinessHour = () => {
    ...
    useEffect(() => {
        if (state?.token) {
            const getOperatingHours = async () => {
                const res = await GetRequest("/operating-hours/partner", state?.token);
    ...
    const handleSave = async () => {
        ...
        const res = await PatchRequest("/operating-hours/partner/bulk", formattedResponse, state?.token);
        if (res?.status === 200 || res?.status === 201) {
                ...
                <View style={{ flex: 1 }}>
                    <View style={{ gap: 12, marginBottom: 40 }}>
                        <View style={styles.headerContainer}>
                            ...
                            <Text style={{ fontFamily:"interSemiBold", fontSize: s(20) }}>Business Hours</Text>
                        </View>
                        <Text style={{ fontSize: s(16), color: "gray" }}>What time do you operate?</Text>
                    </View>
