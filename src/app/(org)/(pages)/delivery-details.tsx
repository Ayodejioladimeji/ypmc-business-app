...
import AssignedSection from "@/components/org-components/assigned-section";
...
const RiderDeliveryDetails = () => {
  const { id, riderId } = useLocalSearchParams()
  ...
  useEffect(() => {
    if (state?.token) {
      const getDelivery = async () => {
        const res = await GetRequest(`/shipping/partner/rider/${riderId}/${id}`, state?.token)
  ...
  const handleRoute = () => {
    dispatch({ type: ACTIONS.ORDER, payload: order })
  ...
  return (
    ...
    >
      ...
      {loading ? <ActivityIndicator />
        :
        <ScrollView style={{ marginTop: 20, paddingHorizontal: 15 }} showsVerticalScrollIndicator={false}>
          <View style={{ flexDirection: "row", marginBottom: 16 }}>
            ...
            <View style={{ marginLeft: 10, flex: 1 }}>
              ...
              <Text style={{ color: "gray", fontSize:s(12) }}>
          ...
          <View style={styles.addressContainer}>
            ...
            <TouchableOpacity
              ...
              onPress={handleRoute}
            >
              <Text style={{ color: "white", fontFamily: 'interSemiBold' }}>Track shipment</Text>
          ...
          <AssignedSection order={order}/>

          <Line />

          <View>
            <Text style={{ fontFamily: 'interMedium', fontSize: s(15), marginTop: 10 }}>Your Earnings</Text>
            <View style={{ flexDirection: "row", justifyContent: "space-between", marginTop: 20 }}>
